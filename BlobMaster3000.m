...
str = {'{\bfHelp: Add ID}';...
    ...
    ['Please note that once added, an individual ID cannot be deleted!',...
    ' The only way to delete IDs is to reset the ', '{\itList of Animals}',...
    ' by re-entering the ', '{\itNumber of Animals}.', ' However, this will', ...
    ' delete the whole tracking progress!']};
...
str = {'{\bfHelp: Assign ID to Location}';...
    ...
    ' checkbox below, multiple consecutive frames can be labeled. For this,',...
...
function rd_help_binary_Callback(hObject, eventdata, handles)
...
str={'{\bfHelp: Delete Position of Current ID}';...
    ...
    ' N animals for the set range of frames.'];...
...
str={'{\bfHelp: Erode-Dilate-Erode Blobs}';...
    [' After applying a threshold, the binary image might be noisy, and blob'...
...
str={'{\bfHelp: Interpolate}';...
    ...
    ' menu. To fill all gaps for all animals, enable the checkbox',...
...
str={'{\bfHelp: Threshold}';...
    ...
    ' threshold is the first step to get well-tracked individuals.', ...
    ' Consequently, it enables following operations. As the video image is',...
    ' converted to grayscale, the range the threshold can be is lies between',...
...
str={'{\bfHelp: Track!}';...
    ...
    ' tracking automatically. Now the user can fast-forward the video until the',...
    ' blobs are separated again, re-assign their identities, go back to the',...
    ' point the tracking stopped, and set a new range for tracking (usually',...
    ' start at the frame the tracking stopped +1). The frame to start with will',...
    ' be filled into the edit box automatically.']};
...
% String to display
str={'{\bfHelp: 2.3(a)  Simple Binarization}';...
    ...
    ' ';
    '{\bfHelp: 2.3(b) Watershed Transform}';...
